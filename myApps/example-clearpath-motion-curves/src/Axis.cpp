...
void Axis::update() {
	if (m_node != NULL) {
		position_cnts.set(ofToString(GetPosition()));
		position_mm.set(ofToString(count_to_mm(GetPosition())));	
	}
}
...
void Axis::on_reset_home_position(bool& val)
{
	if (m_node != NULL) {
		if (val) {
			...
			// turn off the reset home button and update gui
