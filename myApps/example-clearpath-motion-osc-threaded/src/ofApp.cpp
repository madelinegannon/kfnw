...
void ofApp::setup() {
	...
	for (int i = 0; i < controller->get_num_motors(); i++) {
		controller->get_gui(i)->setPosition(x, y);
		x += controller->get_gui(i)->getWidth() + 5;
...
void ofApp::draw() {
	...
	if (use_sine_curve) {
		...
		draw_osc_controller();
		string msg = "Key Count: " + ofToString(key_count) + "\n";
		msg += "Index: " + ofToString(key_count % controller->get_num_motors());
		ofDrawBitmapStringHighlight(msg, ofGetWidth()-100, 50);
	}
...
void ofApp::update_sine_curve()
{
	...
	// update the target position of each motor
	for (int i = 0; i < controller->get_num_motors(); i++) {
		auto origin = toGlm(controller->get_motor(i)->position_world.get());
		//actual_pos = glm::vec3(origin.x, origin.y + actual_dist_mm, 0);
		auto desired_pos = get_projected_point(curve, origin.x);
		auto desired_dist_mm = glm::distance(origin, desired_pos);
		controller->set_target(i, desired_dist_mm);
	}
...
void ofApp::draw_sine_curve()
{
	...
	for (int i = 0; i < controller->get_num_motors(); i++) {
		...
		auto dist_mm = controller->get_motor(i)->count_to_mm(controller->get_motor(i)->GetPosition());
		auto home = toGlm(controller->get_motor(i)->position_world.get());
...
void ofApp::setup_osc_controller()
{
	...
	float step = (controller->get_num_motors() > 1) ? dist / (controller->get_num_motors() - 1) : 0;
	for (int i = 0; i < controller->get_num_motors(); i++) {
		float x = start.x + i * step;
		float y = start.y;
		controller->get_motor(i)->spool.setup(ofVec3f(x, y, 0), 12, 1000, 50);
		//Spool spool;
...
void ofApp::draw_osc_controller()
{
	...
	for (int i = 0; i < controller->get_num_motors(); i++) {
		controller->get_motor(i)->spool.draw();

		// Show ACTUAL position
		ofSetColor(ofColor::red);
		auto dist_mm = controller->get_motor(i)->count_to_mm(controller->get_motor(i)->GetPosition());
		auto home = controller->get_motor(i)->spool.home.getGlobalPosition();
		auto pos = glm::vec3(home.x, home.y + dist_mm, 0);
		ofDrawCircle(pos.x, pos.y, 5);
	}
...
void ofApp::reset_osc_controller()
{
	...
	for (int i = 0; i < controller->get_num_motors(); i++) {
		float x = start.x + i * step;
		float y = start.y;
		controller->get_motor(i)->spool.home.setGlobalPosition(x, y, 0);
	}
}
			...
			// The rest are keys
				...
				for (size_t i = 0; i < m.getNumArgs(); i++) {
					...
					if (start != std::string::npos) {
						...
						int index = 0;
						if (controller->get_num_motors() > 1) {
							int key_code_start = 48;
							int key_code_end = 72;
							int num_steps = key_code_end - key_code_start;

							if (key_code < key_code_start + num_steps / controller->get_num_motors()) {
								index = 0;
							}
							else {
								index = 1;
							...
							// Try just mapping the key strike to the motor
							index = key_count % controller->get_num_motors();
						...
						if (m.getArgType(i) == OFXOSC_TYPE_FLOAT) {
							...
							pressure = ofMap(pressure, 0, 1, 0, controller->get_motor(i)->spool.interval_dist);

							controller->get_motor(index)->spool.pressure_offset = pressure;

							if (pressure == 0)
								key_count++;
						}
						controller->get_motor(index)->spool.set_position(interval);
						controller->set_target(index, controller->get_motor(index)->spool.get_desired_mm());
					}
