...
//	DESCRIPTION:
/**
Get the specified global stop bit.

\param[in] cNum Channel number. The first channel is zero.
\param[out] pEngaged Operational state of the brake.
...
a ClearPath-SC
*/
MN_EXPORT cnErrCode MN_DECL infcGlobalStopGet(
	netaddr cNum,
	nodebool *pEngaged)
{
	// Is the device in our range?
	...
	*pEngaged = !pNCS->pSerialPort->GetCTS();
	...
	BrakeControls brakeVal)
{
	...
	if (SysInventory[cNum].autoBrake[brakeNum].brakeMode == BRAKE_AUTOCONTROL
		...
		&& SysInventory[cNum].autoBrake[brakeNum].relatedNode != MN_UNSET_ADDR) {
